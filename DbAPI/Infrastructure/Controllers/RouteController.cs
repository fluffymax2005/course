...
namespace DbAPI.Infrastructure.Controllers {
    ...
    public class RouteController : BaseCrudController<Core.Entities.Route, TypeId>, ITableState {
        private readonly ILogger<RouteController> _logger;
        private readonly IMemoryCache _cache;

        public RouteController(IRepository<Core.Entities.Route, int> repository, ILogger<RouteController> logger,
            IMemoryCache cache) : base(repository) {
            _logger = logger;
            _cache = cache;
        }

        protected int GetEntityId(Core.Entities.Route entity) {
            return entity.Id;
        }
        ...
        public override async Task<ActionResult<IEnumerable<Core.Entities.Route>>> GetAllAsync() {
        ...
        [HttpGet("{id}")]
        [Authorize(Roles = "Basic, Editor, Admin")]
        public override async Task<ActionResult<Core.Entities.Route>> GetAsync(TypeId id) {
        ...
        public override async Task<IActionResult> CreateAsync([FromBody] Core.Entities.Route entity) {
        ...
        [HttpPut("{id}")]
        [Authorize(Roles = "Editor, Admin")]
        public override async Task<IActionResult> UpdateAsync(TypeId id, [FromBody] Core.Entities.Route entity) {
